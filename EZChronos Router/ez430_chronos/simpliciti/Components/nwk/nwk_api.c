...
#define LINKLISTEN_POLL_COUNT             ((LINKLISTEN_MILLISECONDS_2_WAIT) / \
                                           (LINKLISTEN_POLL_PERIOD_MS))
...
smplStatus_t SMPL_Commission(addr_t *peerAddr, uint8_t locPort, uint8_t rmtPort, linkID_t *lid)
{
    ...
    do {
        if (pCInfo)
        {
            ...
            /* Check port info. */
            if ((locPort > SMPL_PORT_STATIC_MAX) ||
                (locPort < (SMPL_PORT_STATIC_MAX - PORT_USER_STATIC_NUM + 1)))
            {
                continue;
            }

            if ((rmtPort > SMPL_PORT_STATIC_MAX) ||
                (rmtPort < (SMPL_PORT_STATIC_MAX - PORT_USER_STATIC_NUM + 1)))
            {
                continue;
            }
