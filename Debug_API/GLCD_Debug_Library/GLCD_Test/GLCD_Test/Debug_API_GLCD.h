...
public:
    ...
    // Send configs.
    void set_configs(HardwareSerial *serial_device, uint16_t baud, bool flow_control);
    
    // Moves cursor to home position.
    void set_cursor_to_home_pos(void);
    
    // Sets the cursor to the position described - [column] [row]
    void set_cursor_to_pos(uint8_t column, uint8_t row);
    
    // Sets the cursor to a specific pixel - [x] [y]
